...
#include <ctype.h>
#include <unistd.h>
#include "server_neelabhtiwari.h"
...
*/
struct authorizationresult credcheck1(char *uname,char *ps){          
...
/*
    Authorizaton Checker - accepts input of usernumber (OTP) and password
    return a data structure which contains the authorization result and related details (refer server.h)
*/
struct authorizationresult credcheck2(char *unum,char *ps){
  FILE *fptr=fopen("credentials.txt","r");         
  if(fptr==NULL){
    printf("Error in opening the Credentials File\n");
  }
